...
test('renders the table columns for each employee', () => {
  render(
    <EmployeeList employees={[priyaFixture()]} onAddEmployee={jest.fn()} onEditEmployee={jest.fn()} />
  );
...
test('calls onAddEmployee when "+ Add employee" is clicked', () => {
  ...
  render(<EmployeeList employees={[]} onAddEmployee={onAddEmployee} onEditEmployee={jest.fn()} />);
